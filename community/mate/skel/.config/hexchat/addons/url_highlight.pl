...
sub wrap_emit(&) {
	my $block = shift;
	my $strip_color = get_prefs "text_stripcolor";

	if( $strip_color ) {
		command( "SET -quiet text_stripcolor OFF" );
		$block->();
		command( "SET -quiet text_stripcolor ON" );
	} else {
		$block->();
	}
}
...
sub highlight_url {
	...
	strip_code( $text ) if get_prefs "text_stripcolor";
